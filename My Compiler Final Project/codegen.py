...
class TACGen:
    ...
    def expr(self, e):
        if isinstance(e, Number):
            t = self.newt(); self.emit(f"{t} = {e.val}"); return t
        ...
        if isinstance(e, FuncCall):
            # simple: evaluate args and emit CALL (not implemented fully)
            args = [self.expr(a) for a in e.args]
            t = self.newt();
            self.emit(f"{t} = CALL {e.name} {', '.join(args)}")
            return t
        if isinstance(e, BinOp):
            a = self.expr(e.left); b = self.expr(e.right)
            t = self.newt()
            opmap = {"PLUS":"+","MINUS":"-","TIMES":"*","DIVIDE":"/","EQEQ":"==","NE":"!=","LT":"<","GT":">","LE":"<=","GE":">="}
            op = opmap.get(e.op, "?")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 codegen.py sample.src"); sys.exit(1)
